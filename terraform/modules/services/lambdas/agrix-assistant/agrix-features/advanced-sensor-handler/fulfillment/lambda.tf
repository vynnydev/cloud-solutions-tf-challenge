...
resource "aws_lambda_function" "advanced_sensor_handler_fulfillment" {
  function_name    = "advanced_sensor_handler_fulfillment"
  role             = var.agrix_interaction_features_fulfillment_lambdas_roles_iam_arn
  handler          = "advanced_sensor_handler_fulfillment.lambda_handler"
  runtime          = "python3.9"
