...
resource "aws_lambda_function" "ar_processor_handler_fulfillment" {
  function_name    = "ar_processor_handler_fulfillment"
  role             = var.agrix_interaction_features_fulfillment_lambdas_roles_iam_arn
  handler          = "ar_processor_handler_fulfillment.lambda_handler"
  runtime          = "python3.9"
