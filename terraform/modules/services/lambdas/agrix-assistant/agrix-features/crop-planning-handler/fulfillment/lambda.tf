...
resource "aws_lambda_function" "crop_planning_handler_fulfillment" {
  function_name    = "crop_planning_handler_fulfillment"
  role             = var.agrix_interaction_features_fulfillment_lambdas_roles_iam_arn
  handler          = "crop_planning_handler_fulfillment.lambda_handler"
  runtime          = "python3.9"
