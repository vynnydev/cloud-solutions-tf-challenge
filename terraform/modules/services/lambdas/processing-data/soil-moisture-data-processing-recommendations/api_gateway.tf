...
resource "aws_api_gateway_rest_api" "soil_moisture_data_processing_recommendations_api" {
  name        = "SoilMoistureRecommendationsAPI"
...
resource "aws_api_gateway_resource" "recommendations" {
  rest_api_id = aws_api_gateway_rest_api.soil_moisture_data_processing_recommendations_api.id
  parent_id   = aws_api_gateway_rest_api.soil_moisture_data_processing_recommendations_api.root_resource_id
...
resource "aws_api_gateway_integration" "get_recommendations_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.soil_moisture_data_processing_recommendations.invoke_arn
}
