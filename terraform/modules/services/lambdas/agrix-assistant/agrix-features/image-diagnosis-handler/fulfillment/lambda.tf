...
resource "aws_lambda_function" "image_diagnosis_handler_fulfillment" {
  function_name    = "image_diagnosis_handler_fulfillment"
  role             = var.agrix_interaction_features_fulfillment_lambdas_roles_iam_arn
  handler          = "image_diagnosis_handler_fulfillment.lambda_handler"
  runtime          = "python3.9"
