# API Gateway
resource "aws_api_gateway_rest_api" "brightness_task_planner_api" {
  name        = "AirBrightnessTaskPlannerAPI"
  description = "API for task planner application"
}

# Recursos da API Gateway
resource "aws_api_gateway_resource" "brightness_task_plan" {
  rest_api_id = aws_api_gateway_rest_api.brightness_task_planner_api.id
  parent_id   = aws_api_gateway_rest_api.brightness_task_planner_api.root_resource_id
  path_part   = "task-plan"
}
...
resource "aws_api_gateway_method" "get_task_plan" {
  rest_api_id   = aws_api_gateway_rest_api.brightness_task_planner_api.id
  resource_id   = aws_api_gateway_resource.brightness_task_plan.id
  http_method   = "GET"
  authorization = "NONE"
}
...
resource "aws_api_gateway_integration" "lambda_task_plan" {
  ...
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.brightness_task_planner.invoke_arn
}